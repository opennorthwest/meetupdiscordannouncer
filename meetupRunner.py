...
for event in events:
    ...
    if event_date.date() <= now.date() + datetime.timedelta(days=7 if is_sunday() else 1):
        messages.append(build_weekly_msg(event, event_date) if is_sunday() else build_message(event))
